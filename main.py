...
from pwl_data2  import TriangularPWLData   # <<< NEW: import triangular generator
...
def main():
    ...
    parser.add_argument('--optimizer',
                        choices=['adam','sgd','rmsprop','adagrad','adadelta'],
                        default='adam')
    ...
    # <<< NEW: helper to choose between 1D and 2D generators
    def make_dataset(mode):
        """
        mode: 'train' | 'val' | 'test' | 'plot'
        """
        if mode == 'plot':
            ...
            n_samples = {'train': args.n_train,
                         'val':   args.n_val,
                         'test':  args.n_test}[mode]
            inject    = args.inject if mode == 'train' else 0
            seed      = args.seed + {'train':0, 'val':1, 'test':2}[mode]

        if args.in_dim == 2:
            # TriangularPWLData ignores breakpoints_x/y and inject
            return TriangularPWLData(
                breakpoints_x=None,
                breakpoints_y=None,
                n_samples=n_samples,
                inject=None,
            ...
            # Classic 1D PWLData
            return PWLData(
                breakpoints_x=args.breakpoints_x,
                breakpoints_y=args.breakpoints_y,
                n_samples=n_samples,
                inject=inject,
                seed=seed,
                in_dim=args.in_dim
    ...
    # -- datasets & loaders --
    train_ds = make_dataset('train')   # <<< UPDATED
    val_ds   = make_dataset('val')     # <<< UPDATED
    test_ds  = make_dataset('test')    # <<< UPDATED
    loaders = [
    ...
    if args.n_samples is not None:
        plot_ds = make_dataset('plot')  # <<< UPDATED
